...
def get_data(driver, data_list, frame_list, records):
    try:
        WebDriverWait(driver, 60).until(EC.presence_of_element_located
                                        ((By.CSS_SELECTOR, '#ContentPlaceHolder1_gdvDeadBody')))

        logger.info(f"{unit_names[counter]}: {records}")
    except NoSuchElementException:
    ...
    frame_list.append(pd.DataFrame(data_list, columns=COLUMNS))
def toggle_pages(driver, district):
    page_data = []
    district_dataframe = []
    # define a variable for link_text parameter
    link_text_counter = 0
    # add code to see if new page has started.
        # some code here.
    # how many records are available?
    WebDriverWait(driver, 60).until(EC.presence_of_element_located((
        By.CSS_SELECTOR, '#ContentPlaceHolder1_lbltotalrecord')))
    number_of_records_found = driver.find_element_by_css_selector(
        '#ContentPlaceHolder1_lbltotalrecord').text
    total_pages = number_of_records_found//50
    page_slots = total_pages//10
    page_slot_elements = driver.find_elements_by_css_selector('.gridPager a')
    for page in page_slot_elements:
        # to avoid stale element exception
        page_slot_elements_again = driver.find_elements_by_css_selector(
            '.gridPager a')
        if page.text != '...':
            page.click()
            
    while page_slots >= link_text_counter:
        # download the records at current page (normaly #1, #11...)
        page = 0
        while page < len(pages_current_slot)-1:
            wait_for_page_load(driver, 60)
            get_data(driver, page_data, district_dataframe, number_of_records_found)
            pages_current_slot_new = driver.find_elements_by_css_selector('.gridPager a')
            link_text = pages_current_slot_new[page].text
